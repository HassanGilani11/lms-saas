...
import { Trash2, Folder, MoreVertical, Plus, Pencil, Search, ChevronLeft, ChevronRight, Tags } from "lucide-react";
...
const GroupCategoriesPage = () => {
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        try {
            if (editingCategory) {
                await updateGroupCategory(editingCategory.id, values);
                toast.success("Category updated successfully");
            } else {
                await createGroupCategory(values);
    ...
    const onEdit = (category: any, e: React.MouseEvent) => {
        e.stopPropagation();
                    ...
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-4">
                            ...
                            <Button type="submit" className="w-full bg-slate-900 text-white hover:bg-slate-800 h-10 font-bold mt-4" disabled={form.formState.isSubmitting}>
                                {form.formState.isSubmitting ? "Saving..." : (editingCategory ? "Save Changes" : "Create Category")}
                            </Button>
                        </form>
                    </Form>
                </DialogContent>
...
export default GroupCategoriesPage;
