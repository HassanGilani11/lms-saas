...
import { Trash2, Tag, MoreVertical, Plus, Pencil, Search, ChevronLeft, ChevronRight, Filter, ArrowUpDown } from "lucide-react";
import {
    DropdownMenu,
    DropdownMenuContent,
    DropdownMenuItem,
...
const InstructorCourseTagsPage = () => {
    ...
    return (
        <div className="p-6 text-slate-900 dark:text-slate-100 font-sans space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-[17px] font-bold text-slate-800 dark:text-slate-100">Course Tags</h1>
                    <p className="text-xs text-slate-500 mt-1">Manage tags for your courses</p>
                </div>
            </div>
                ...
                <CardContent className="p-0">
                    <Table>
                        <TableHeader>
                            <TableRow className="bg-slate-50/50 dark:bg-slate-800/50 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800">
                                <TableHead className="pl-6 font-bold text-slate-400 dark:text-slate-500 text-[11px] uppercase tracking-tighter">Tag Name</TableHead>
                                <TableHead className="font-bold text-slate-400 dark:text-slate-500 text-[11px] uppercase tracking-tighter text-center">Courses Count</TableHead>
                                    ...
                                    <TableCell className="pl-6 py-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex flex-col">
                                                <span className="text-[13px] font-bold text-slate-900 dark:text-slate-100">{tag.name}</span>
                                            </div>
                                        </div>
                                    </TableCell>
                    ...
                    {(filteredTags.length === 0 || isLoading) && (
                        <div className="text-center py-24 text-slate-400 flex flex-col items-center gap-y-2">
                            <Tag className="h-10 w-10 text-slate-100 dark:text-slate-800" />
                    ...
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-4">
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel className="text-[13px] font-bold text-slate-700 dark:text-slate-300">Tag Name</FormLabel>
                                        <FormControl>
                                            <Input placeholder="e.g. New, Bestseller, Technical" className="h-10 text-[13px]" {...field} />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}
...
export default InstructorCourseTagsPage;
